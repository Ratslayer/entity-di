...
public static class ListUtils
{
	...
	public static (TElement _element, float minValue, TResult _result) GetMin<TElement, TResult>(
		this IList<TElement> list,
		Func<TElement, (float, TResult)> comparer)
	{
		switch (list.Count)
		{
			case 0:
				return default;
			case 1:
				var (minValue, result) = comparer(list[0]);
				return (list[0], minValue, result);
			default:
				(minValue, result) = comparer(list[0]);
				var element = list[0];
				for (var i = 1; i < list.Count; i++)
				{
					var (newValue, newResult) = comparer(list[i]);
				...
				return (element, minValue, result);
		}
	}
